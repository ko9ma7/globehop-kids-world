const EN = {
  ...
  dataSourceBody: 'Core country facts are bundled as static data. City search uses Open-Meteo when available, statistics use World Bank, road estimates can use OSRM, and landmark thumbnails are requested from Wikipedia/Wikimedia page images. Plane arcs are educational visualizations, not actual airline paths.',
  close: 'Close',
  theme: 'Theme',
  light: 'Light',
  dark: 'Dark',
...
const STRINGS = {
  ...
  ko: {
    ...
    dataSourceBody: '국가 기본 데이터는 프로젝트에 포함된 정적 데이터입니다. 도시 검색은 Open-Meteo, 최신 통계는 World Bank, 가능한 육상 경로는 OSRM, 명소 이미지는 Wikipedia/Wikimedia 페이지 이미지를 보조적으로 사용합니다. 비행기 호는 실제 항공 노선이 아니라 학습용 시각화입니다.',
